...
def test_build_add_main() -> None:
    assert build_add(["httpx"], group="main") == ["uv", "add", "httpx"]


def test_build_add_multiple_main() -> None:
...
def test_build_add_script_single() -> None:
...
def test_build_add_script_multiple_preserves_spec() -> None:
...
def test_build_remove_script() -> None:
...
def test_build_run_script() -> None:
    assert build_run_script("demo.py") == ["uv", "run", "--script", "demo.py"]


def test_build_run_with_args_no_separator() -> None:
...
def test_build_run_script_with_args() -> None:
...
def test_build_run_defaults_to_no_args() -> None:
    assert build_run("serve") == ["uv", "run", "serve"]


def test_build_tree_bare() -> None:
    assert build_tree() == ["uv", "tree", "--format", "json", "--frozen"]


def test_build_tree_outdated() -> None:
...
def test_build_tree_not_frozen() -> None:
    assert build_tree(frozen=False) == ["uv", "tree", "--format", "json"]


def test_build_tree_scoped_to_package() -> None:
...
def test_build_add_optional_group() -> None:
...
def test_build_remove_optional() -> None:
...
def test_reserved_name_extra_routes_by_kind() -> None:
...
def test_build_add_dependency_group() -> None:
...
def test_build_remove_dependency_group() -> None:
...
def test_build_add_optional_extra_kind() -> None:
    assert build_add(["typer"], group="cli", kind="extra") == [
        "uv",
        "add",
        "--optional",
...
def test_build_sync_lock_run() -> None:
    assert build_sync() == ["uv", "sync"]
    assert build_lock() == ["uv", "lock"]
    assert build_run("serve") == ["uv", "run", "serve"]


def test_build_sync_scoped() -> None:
...
def test_build_sync_partial_options() -> None:
...
def test_build_python_commands() -> None:
...
@pytest.mark.asyncio
async def test_run_capture_returns_exit_code_and_output() -> None:
...
@pytest.mark.asyncio
async def test_run_capture_excludes_stderr() -> None:
...
@pytest.mark.asyncio
async def test_run_capture_reaps_on_cancel() -> None:
...
@pytest.mark.asyncio
async def test_run_streaming_captures_lines_and_exit_code() -> None:
    lines: list[str] = []
...
@pytest.mark.asyncio
async def test_run_streaming_nonzero_exit() -> None:
...
@pytest.mark.asyncio
async def test_run_streaming_reaps_on_cancel() -> None:
    argv = [sys.executable, "-c", "import time; time.sleep(30)"]
...
def test_uv_available_true(monkeypatch) -> None:
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv")
    assert uv_available() is True


def test_uv_available_false(monkeypatch) -> None:
    monkeypatch.setattr("shutil.which", lambda name: None)
    assert uv_available() is False


def test_build_tool_commands() -> None:
    from lazyuv.commands import (
        build_tool_install,
        build_tool_list,
...
def test_build_cache_and_self_commands() -> None:
    from lazyuv.commands import (
        build_cache_clean,
        build_cache_dir,
        build_cache_prune,
...
def test_build_lock_upgrade_package() -> None:
...
def test_build_export_default_and_options() -> None:
